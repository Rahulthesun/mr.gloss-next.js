import React, { useEffect, useRef, useCallback, useMemo, ReactNode, CSSProperties } from "react";
...
const TiltCard: React.FC<TiltCardProps> = ({ children, className = "", style = {}, enableTilt = true }) => {
  ...
  return (
    <div ref={wrapRef} className={`tiltcard-wrapper ${className}`.trim()} style={style}>
      <section ref={cardRef} className="tiltcard-card">
        {children}
      </section>
      {/* Inline styles for tilt effect (can be moved to CSS module if desired) */}
      <style>{`
        .tiltcard-wrapper {
          perspective: 1200px;
          --pointer-x: 50%;
